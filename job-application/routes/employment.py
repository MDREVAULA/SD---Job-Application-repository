    ...
    if request.method == 'POST':
        ...
        for req in requirements:
            ...
            if sub:
                ...
                sub.submitted_at = datetime.get_ph_time()
                sub.notes        = None
            else:
        ...
        if uploaded_any:
            ...
            if onboarding:
                ...
                onboarding = EmploymentOnboarding(
                    application_id=app_id,
                    status='submitted',
                    submitted_at=datetime.get_ph_time()
                )
...
def fire_employee(employee_id):
    ...
    if emp.resignation_request:
        ...
        emp.resignation_request.reviewed_at   = datetime.get_ph_time()
...
def review_resignation(resignation_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'approve':
            ...
            resignation.reviewed_at   = datetime.get_ph_time()
...
def submit_resignation(employee_id):
    ...
    if request.method == 'POST':
        ...
        if existing and existing.status in ('rejected', 'revision_requested'):
            ...
            existing.submitted_at      = datetime.get_ph_time()
            if letter_filename:
